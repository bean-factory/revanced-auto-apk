...
patch_apk() {
	...
	local cmd="java -jar $RV_CLI_JAR --temp-dir=$tdir -c -a "$stock_input" -o "$patched_apk" -b $RV_PATCHES_JAR --keystore=ks.keystore -i predictive-back-gesture $patcher_args --options=./options.toml"
	echo "$cmd"
	eval "$cmd"
}
